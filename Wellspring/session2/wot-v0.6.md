...
**Domains:** wot.rocks · wot.technology · now.pub
...
**Attention sovereignty**: private checkpoints track your own engagement without ever leaving your device. Platforms harvest your cognitive exhaust to optimize their goals. WoT lets you harvest it to optimize yours.
...
Not all engagement needs to be public. Private checkpoints track your own attention without ever leaving your device.

```rust
Thought {
    type: "checkpoint",
    about: [article_cid],
    visibility: "private",  // never syncs, never leaves
    content: {
        opened: true,
        scroll_depth: 0.8,
        time_spent: "4m32s",
...
| Public attestation | Private checkpoint |
|-------------------|-------------------|
| "I vouch for this" | "I looked at this" |
| Affects others' trust | Only affects MY salience |
| Reputation on the line | No one knows |
| Deliberate act | Ambient collection |

**Feeds the heat model:**

```
private_engagement_score = (
    opens +
    return_visits × 2 +
...
salience += private_weight × private_engagement_score
```

You never tell anyone you read something 5 times. But YOUR index knows. That thing surfaces when relevant.

**The honest index:**

```
Public:  "I recommend this article"
Private: Opened 12 times, never finished, always bounced at section 3

Your salience: LOW despite public recommendation
Your behavior contradicts your attestation.
Index knows the difference.
```

**Flip the surveillance model:**

```
Platform model:
  Track your attention → Sell to advertisers → Optimize THEIR goals

WoT model:
  Track your attention → Keep private → Optimize YOUR goals
```
...
Thought {
    ...
    content: {
        ...
        return_patterns: {
            pricing_bug: { visits: 12, resolved: false },
            wot_spec: { visits: 47, progressing: true },
            twitter: { visits: 89, attestations_produced: 0 },
        },
        attention_debt: {
            opened_never_finished: 34,
            saved_never_opened: 89,
        },
    },
}
```

The uncomfortable mirror: "You say family is priority #1. Your attention checkpoints show 2h/week. Work shows 34h/week."

Not judgment. Just data. YOUR data. Never shared unless you choose.

**Reclaim the exhaust:**

Every click, scroll, hover, return visit, time spent — cognitive exhaust. Platforms harvest it.

WoT: you harvest it. Private checkpoints. Never leaves. Feeds your salience. Shows you your actual mind, not the one you pretend to have.

> "Know thyself" — with receipts that no one else sees.
...
The `/now` page movement (Derek Sivers, 2015) wanted people to share what they're focused on *right now*, not just static /about pages. 2000+ people maintain /now pages manually. The movement hit limits: manual updates, no standard format, no verification, stale pages everywhere.
...
keif.now.pub          → Current identity + focus + status
buymaterials.now.pub  → Company identity + availability
agent-7.now.pub       → Bot identity + operating state
```
...
Thought {
    type: "now",
    identity: keif_identity_cid,
    content: {
        focus: [wot_spec_cid, buymaterials_pricing_cid],
        status: "deep work",
        available: false,
        location: "UK",  // optional
    },
    ttl: "24h",  // auto-stale if not refreshed
    signature: "...",
}
```
...
| Old /now page | now.pub |
|---------------|---------|
| Manual updates | Ambient from trails |
| No format | Thought schema |
| Not machine readable | CID + structured content |
| No verification | Signed, timestamped |
| No identity link | now.pub IS the identity anchor |
| Stale pages | TTL expiry, auto-warning |

**DNS setup:**

```
keif.now.pub        CNAME  [your hosting]
_wot.keif.now.pub   TXT    "v=wot1 root=... id=did:key:..."
```

**Real-time identity resolution:**

```
"Who is keif right now?"
  → keif.now.pub
  → Current identity anchor
  → Current pubkey
  → Current focus (with trails)
  → Current status

Not who they WERE. Who they ARE.
```
...
Thought {
    type: "now",
    identity: agent_7_cid,
    content: {
        status: "processing",
        queue_depth: 7,
        estimated_available: "5m",
        constraints: [operating_rules_cid],
    },
}
```
...
```
wot.rocks       → Project home, what this is
wot.technology  → Technical docs, how it works  
now.pub         → Live identities, who's here now
```

DNS for lookup. now.pub for broadcast. The protocol is complete.
...
│    now.pub   — live identity broadcast namespace        │
...
│                                                         │
│  Private checkpoints (attention sovereignty):           │
│    Track your own engagement — never leaves device      │
│    Feeds YOUR salience, not advertisers                 │
│    Honest index: behavior vs stated beliefs             │
...
- **Rework chain structure and capture modes** — Thoughts have two parallel chains: `because_chain` (why this exists — sources, context, prior thoughts) and `rework_chain` (how this became this — edit history from keystroke to final). Capture mode spectrum: `final` (published version only), `major` (significant edits), `edits` (all save points), `keystream` (every keystroke). Open questions: (1) What's the wire format for keystroke capture? Timestamps, key codes, or diff stream? (2) How does rework_chain interact with corrections model — correction proposals vs inline edits? (3) Storage implications — keystream could be 100x the size of final content. (4) Privacy of process — do you want people seeing your hesitations?

- **Pool capability announcement and client conformance** — Pools declare capture requirements on entry: "This pool requires keystroke-level capture. Thoughts without rework_chain will be rejected." Clients must signal what they support. Open questions: (1) How are pool requirements encoded? YAML manifest? Self-describing thought at pool root? (2) How do clients advertise capabilities? (3) What happens when a client can't meet requirements — silent rejection or negotiation? (4) Can pools have tiered requirements (keystream preferred, edits minimum, final rejected)?

- **OLP-style structural health metrics** — Integrate OpenLine Protocol concepts for pre-verification triage: `cycle_plus` (circular reasoning), `ucr` (unsupported claim ratio), `s_over_c` (support-to-claim ratio), `κ` (stress — graph growing faster than verification). Open questions: (1) WoT gets cycle-freedom for free from CID addressing — is cycle_plus redundant? (2) How to compute UCR — thoughts with empty because[] vs some threshold? (3) Should pools set structural health minimums as admission gates? (4) How does OLP triage feed into subconscious agent prioritization?
