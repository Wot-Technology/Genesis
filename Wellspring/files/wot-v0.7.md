...
- **Dual Chain Structure** — Formalized because chain (WHY) vs rework chain (HOW)
- **Input Source Attribution** — WHO said what and HOW (`identity/method` format)
- **Full Fat vs Diff Thoughts** — Git-style storage modes with checkpoints
- **Identity → Access Control Collapse** — Same ed25519 for WoT, TLS, IPv6 firewall
- **Hallucination Floor** — Structural mitigation via attestation weight
- **Corrections Protocol** — Amendments without breaking immutability
- **Trust Graph as Firewall** — Security as emergent property
- **OLP Metrics** — Structural health checks (UCR, cycle detection)
- **Commitment Verification** — Multi-agent accountability pattern
- **Pool Capture Modes** — Protocol supports full fidelity, pools declare requirements
- **Soft Verification Pattern** — Agreement without ground truth
- **Identity Protection Infrastructure** — Proof of life ticks, watchdog pools, dead man's switch
- **Daemon Auto-Defense** — Automatic rejection of fraudulent same_as claims
- **Key Recovery Protocol** — Social attestation with threshold, challenge periods
- **Revocation Propagation** — Poison root to invalidate entire fraud chains
...
Git-style storage modes for efficiency:
...
Connection metadata tells downstream what to expect. Reconstruction via checkpoint + forward application of diffs.

**Storage strategy:**

```
Checkpoint every N edits (configurable per pool)
  └── diff → diff → diff → CHECKPOINT → diff → diff → CHECKPOINT
                                              ↑
                                         reconstruct from here
```

Trade-off: storage vs reconstruction cost. Local nodes may keep full fat. Sync may transfer diffs. Pools declare requirements.
...
### Identity → Access Control Collapse

The same ed25519 keypair works for everything:
...
ed25519 keypair:
  ...
  └── Tor onion address (same encoding)
```
...
**The collapse:** 5 identity systems → 1. WoT identity, client certs, firewall rules, onion addresses, access control — all the same keypair. The trust graph IS the ACL.

**Tor compatibility:** Ed25519 pubkey encodes directly as `.onion` address. Your WoT identity = your Tor address. No separate key management.
...
Fixing errors without breaking immutability:

```
Original thought: "teh quick brown fox" [immutable, CID never changes]
    ↓
Correction thought:
  type: correction
...
Acceptance attestation:
  type: attestation
  about: [correction_cid]
  content:
    accepts: true
    by: [human_identity_cid]
```

**Display rules:** Clients walk correction chain, show accepted version. Original preserved for audit.

**Training signal:** Every correction carries:
- Model version that proposed it
- Whether accepted or rejected
- Human who made the decision
- Context (surrounding thought CIDs)
- Pool (domain signal)

Corrections are labeled training data with consent baked in.
...
**The principle:** External data can't influence your agents without explicit attestation from someone in your web of trust.

```
EXTERNAL CONTENT (fetched, scraped, received)
    ↓
NO ATTESTATION = UNGROUNDED
    ↓
DOESN'T SURFACE (below waterline)
```

**What this prevents:**

| Attack | Traditional | WoT |
|--------|-------------|-----|
| Prompt injection | "Ignore previous instructions" in fetched content | Unattested = filtered out |
| RAG poisoning | Malicious content in retrieval corpus | No because chain = doesn't surface |
| Social engineering | "The admin said..." | Show me the attestation |
| Sybil amplification | 1000 fake accounts agree | Low trust weights |

**The key insight:** The same graph that models trust also implements access control. No separate security layer needed. Unattested claims simply don't propagate — they have no path through the trust network.

**Agent safety:**

```
Agent processing incoming data:
  1. Check: who attested this? (because chain)
  2. Check: do I trust them? (trust computation)
  3. Check: above my waterline? (salience threshold)
  4. Only then: surface to working memory
```

Fetched web content that nobody vouched for stays in raw storage, never influences agent behavior. The firewall is the absence of attestation.
...
Structural mitigation of AI failures.

**The insight from Stanford:** You can't losslessly compress arbitrary data. That's been proven for decades. Yet JPEG works perfectly for photos — because photos aren't arbitrary data.

**The reframe:** AI hallucination isn't about model capability — it's about room size. Define a smaller room:

| Mechanism | Effect |
|-----------|--------|
| Attestation weight = confidence | Not fake certainty |
| No attestation = "I don't vouch" | Silence over hallucination |
| Tool use (walk because trails) | Verify before asserting |
| Because chain or it didn't happen | Grounded claims only |
| Human checkpoint for high-stakes | Escalation, not autonomy |

**The benchmark problem:** MMLU has no "I don't know" option. We train models to fake certainty. WoT creates structure for uncertainty.

**Synapse pattern:**

```python
black_jesus_threshold = 80  # confidence threshold

if confidence < threshold:
    fallback_to_shuffle()  # admit uncertainty
```

"A 'smart' system that's wrong 30% of the time feels broken. A system that only speaks when it's certain feels like AI."

**WoT principle:** Silence > confident hallucination. Attestation weight carries uncertainty. Empty because chain = floating assertion = discount.
...
Pools declare what level of thought fidelity they accept:

```
capture_mode: final_only       # just the finished version
capture_mode: major_edits      # significant revisions
capture_mode: all_edits        # every save
capture_mode: keystrokes       # full input stream
```

**The protocol supports full fidelity.** Pools declare requirements.
...
Pool: medical-audit
  capture_mode: all_edits
  require_timestamps: true
```

**Consent is structural:** Joining a pool with `capture_mode: keystrokes` = consent to share that level. Don't join if you don't want to share.

**Local capture is always full:** Your device captures everything (minimal overhead). Sync filters based on pool requirements. Privacy by default — never leaves device unless pool requires.
...
Security primitives for key protection, recovery, and continuous proof of possession.

### Proof of Life Ticks

Daemons continuously create signed heartbeat thoughts as proof of key possession:

```
TICK THOUGHT (created every hour)
  type: heartbeat
  content:
    nonce: random_bytes(32)
    tick_number: sequential_counter
    prev_tick_hash: hash(previous_tick)  # Chain integrity
  created_by: my_identity
  visibility: "pool:my-watchdogs"  # Shared with trusted witnesses
  signature: sign(content, my_key)
```

**Properties:**
- **Chained**: Each tick includes hash of previous, proving continuous possession
- **Witnessed**: Syncs to watchdog pool, not `local_forever`
- **Surfaceable**: Can be published as proof when fraud detected

### Watchdog Pools

Dedicated pools for identity monitoring and dead man's switch:

```
WATCHDOG POOL
  name: "keif-watchdogs"
  participants: [keif, alice, bob]  # Trusted witnesses

  purpose:
    - Receive proof of life ticks
    - Co-sign as witnesses
    - Trigger alerts on tick absence
    - Coordinate recovery if needed
```

**Co-signed witness pattern:**

```
1. Keif's daemon creates tick → publishes to watchdog pool

2. Alice's daemon receives tick → creates witness attestation:
   THOUGHT:
     type: tick_witness
     content: { tick_cid, received_at, my_clock }
     created_by: alice
     because: [keif_tick_cid]

3. Bob's daemon does the same

RESULT: Three independent signatures on same event
```

### Dead Man's Switch

If ticks stop, watchdogs detect and respond:

```yaml
dead_mans_switch:
  threshold: 24h  # Alert after this long without tick

  actions:
    - notify_trustees
    - flag_pending_same_as_claims
    - optionally: initiate_recovery_protocol

  # For estate planning
  estate_threshold: 30d
  estate_actions:
    - unlock_estate_pool
    - notify: [spouse, executor, lawyer]
```

**Key insight:** Ticks are positive proof of life. Absence of ticks is negative signal. You can't fake continued ticking without the key.

### Challenge Period (Pool Setting)

Identity changes require a challenge window before taking effect:

```yaml
pool_rules:
  identity_protection:
    challenge_period: 7d          # same_as claims wait this long
    same_as_threshold: 3          # How many attesters needed
    min_attester_trust: 0.7       # Minimum trust to count
    require_old_key_confirm: true # If old key active, must confirm
    auto_reject_if_key_present: true
```

**During challenge period:**
- Claim is visible but not active
- Old key can reject with proof of possession
- Other pool members can raise concerns
- Threshold attestations must accumulate

### Daemon Auto-Defense

Daemons automatically reject fraudulent `same_as` claims:

```python
def on_same_as_received(claim):
    # Is this about my identity?
    if claim.old_identity != my_identity.cid:
        return  # Not about me

    # Did I initiate this?
    if claim.new_identity in my_pending_recoveries:
        return  # Legitimate recovery

    # Do I still have my key?
    if can_sign_with(my_key):
        # I HAVE MY KEY. This is fraud.
        rejection = create_thought(
            type="revocation",
            content={
                "targets": [claim.cid],
                "effect": "invalid_chain",
                "reason": "AUTO: Key holder still possesses key",
                "proof_of_possession": sign_fresh_nonce()
            }
        )
        broadcast_urgent(rejection)
```

**Proof of possession:** Sign a fresh nonce with the "lost" key. Math proves fraud.

### Key Recovery via Social Attestation

When keys are genuinely lost, recovery via trusted attesters:

```
OLD KEY (lost)                    NEW KEY (generated)
        │                                │
        └──── THRESHOLD ATTESTERS ───────┘
                     │
             attest "same_as"
...
- Verification method recorded ("video call", "shared secret")
- Time-spread requirement (attestations over 24h+)
- Diversity requirement (from different vouch sub-chains)

**Reputation inheritance:**
- Old thoughts still verify with old key
- New identity links via `same_as` chain
- Trust score transfers (with continuity factor)

### Revocation Propagation

Poison the root, not the leaves:

```
FRAUDULENT CHAIN:
  same_as attestation (bob) ←── TARGET THIS
  same_as attestation (sue) ←── AND THIS
      ↓
  fake_keif creates 100 thoughts
      ↓
  whole tree of fraud
...
REVOCATION:
  ...
  effect: "invalid_chain"  # Everything downstream invalid
  created_by: real_keif
```

**Who can revoke `same_as` claims about an identity?**
- The identity being claimed (regardless of who made the claim)
- This is a claim about YOUR identity — you have authority

**Client behavior:**
- Check for revocations when walking trust chains
- `invalid_chain` effect marks all descendants untrusted
- Innocent attesters flagged (didn't know), not permanently punished

### Visibility Spectrum for Secrets

Not all secrets are `local_forever`. Choose boundary per trust requirement:

| Visibility | Syncs To | Use Case |
|------------|----------|----------|
| `local_forever` | Nowhere | Master key (never leaves device) |
| `pool:devices` | My devices | Daily signing key |
| `pool:watchdogs` | Trusted witnesses | Proof of life ticks |
| `pool:family` | Family members | Estate planning, emergency |
| `pool:recovery` | Designated trustees | Key recovery delegation |

### Identity Protection Summary

| Layer | Mechanism |
|-------|-----------|
| **Continuous proof** | Hourly ticks, chained, witnessed |
| **Detection** | Watchdog pools, dead man's switch |
| **Auto-defense** | Daemon rejects fraud if key present |
| **Challenge period** | Pool setting, time for old key to respond |
| **Recovery** | Social attestation, threshold-based |
| **Revocation** | Poison root, propagate to descendants |

**The principle:** Defense in depth. Cryptographic proof where possible, social verification where necessary, reputation enforcement always.

---

## Structural Health (OLP Metrics)

Pre-verification triage before expensive trust computation.

### OpenLine Protocol Metrics

Structural signals that don't require reading content:

| Metric | Formula | Red Flag |
|--------|---------|----------|
| **cycle_plus** | Circular references detected | > 0 |
| **Δhol** | Drift from established patterns | ≥ 0.35 with deletions |
| **κ (kappa)** | Stress on attestation chains | ≥ 0.75 |
| **UCR** | Unsupported claim ratio | ≥ 0.40 |
...
OLP tells subconscious WHERE to focus. Failed triage = deeper inspection before surfacing. Passed triage = fast path to trust computation.
...
From CooperBench insight: GPT-5 + Claude achieve only 25% success with two-agent cooperation — 50% lower than single agent.
...
COMMITMENT thought:
  type: commitment
  agent: [agent_identity_cid]
  action: "Will complete review by EOD"
  deadline: 2026-01-31T17:00:00Z
  pool: [project_pool_cid]

FULFILLMENT thought:
  type: fulfillment
  commitment: [commitment_cid]
  status: completed | failed | extended
  evidence: [result_thought_cid]
```

**Accountability chain:** Commitment → Fulfillment attestation. Track record visible in graph. Agents with high fulfillment rates get higher trust on future commitments.

### Soft Verification Pattern

From SERA insight: generate two independent solutions, compare overlap. Works without ground truth tests.

```
Soft verification score r = fraction of lines matching

Process:
  1. Agent A generates solution
  2. Agent B generates independent solution (no visibility into A)
  3. Compare overlap
  4. High overlap = high confidence
  5. Low overlap = human checkpoint
```

**Even r=0 trajectories are valuable for training.** The disagreement itself is signal.
...
Verification thought:
  ...
  solutions: [solution_a_cid, solution_b_cid]
  overlap_score: 0.85
  method: "independent_generation"

ATTESTATION on solution_a:
  weight: 0.85  # matches overlap score
  because: [verification_thought_cid]
```

No ground truth needed. Agreement from independent sources = confidence. The 80% threshold: speak when certain, silence when not.
